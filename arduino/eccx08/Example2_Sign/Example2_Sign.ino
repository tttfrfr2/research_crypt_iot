...
void setup() {
  ...
  if (!atecc.loadTempKey(message)){
    Serial.println("loadTempKey failed");
  } else if (!atecc.signTempKey(0)) {
    Serial.println("signTempKey failed"); // by default, this uses the private key securely stored and locked in slot 0.
  }
