...
{
  _i2cPort->beginTransmission(0x60); // set up to write to address "0x00",
...
*/
boolean ATECCX08A::aesEncryptEBC(uint16_t slot, uint8_t *data1, uint8_t *encrypted) {
  if (!sendCommand(COMMAND_OPCODE_AES_EBC, AES_MODE_ENCRYPT_POSITION0, slot, data1, 16)) {
    _debugSerial->println("sendCommand of aesEncryptEBC ERROR");
    return false;
  }

  delay(50);

  if (!receiveResponseData(RESPONSE_COUNT_SIZE + RESPONSE_AES_EBC_SIZE + CRC_SIZE, true)){
    _debugSerial->println("receiveResponseData of aesEncryptEBC ERROR");
    return false;
  }
  ...
  if (!checkCrc()){
    _debugSerial->println("checkCrc of aesEncryptEBC ERROR");
    return false;
  } else if ( !checkCount()){
    _debugSerial->println("checkCount of aesEncryptEBC ERROR");
    return false;
  }

  for (int i = 0; i < RESPONSE_AES_EBC_SIZE; i++) {
    encrypted[i] = inputBuffer[i + RESPONSE_COUNT_SIZE];
  }
...
*/
boolean ATECCX08A::aesDecryptEBC(uint16_t slot, uint8_t *data1, uint8_t *decrypted) {
  if (!sendCommand(COMMAND_OPCODE_AES_EBC, AES_MODE_DECRYPT_POSITION0, slot, data1, 16)) {
    return false;
  }
  ...
  for (int i = 0; i < RESPONSE_AES_EBC_SIZE; i++) {
    decrypted[i] = inputBuffer[i + RESPONSE_COUNT_SIZE];
  }
