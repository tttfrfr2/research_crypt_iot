...
char gen_key_block(int *key_index, int key_length, int *key){
        char block;
        block = 0;
        for (int i = 7; i > 0; i--) {
...
char encrypt(char *message, int *key_index, int key_length, int *key, int MessLen) {
        for (int messInd = 0; messInd < MessLen; messInd++) {
                char keyBlock = gen_key_block(key_index, key_length, key);
                message[messInd] = char(message[messInd] ^ keyBlock);
        }
